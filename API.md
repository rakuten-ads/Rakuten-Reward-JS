...
| tokenType                      | 'default', 'omni | Optional  | 'default'     | Token Type used for Mission SDK                                                                                                                  |
...
| userName                       | string           | Optional  | ''            | User's name, used to display user's name in SDK Portal. If not empty, user's full name is displayed in the Portal Popup                          |
...
| omniRefreshAccessTokenFunction | Promise<string>  | Optional  | undefined     | Function to be called if you're using OMNI Access Token and the token expired. The function should support Promise and return a new access token |
|                                |                  |           |               |                                                                                                                                                  |
...
| `error_no_omni_refresh_access_token_function` | You don't provide a refresh access token function                     | You haven't provided a function to refresh the access token automatically. So please handle refresh access token from your side.                                                                                                             |
